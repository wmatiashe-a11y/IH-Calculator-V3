...
class Overlays:
    ...
    # Split marketing + finance (recommended)
    marketing_rate: float = 0.03
    finance_rate_on_costs: float = 0.015  # ✅ calibrated default (NOT 0.10)

    # Legacy blended (older UI/code paths)
...
with st.sidebar.expander("Site & Zoning", expanded=True):
    ...
    assumed_floors = st.number_input("Assumed floors (coverage cap)", min_value=1, value=3, step=1)
    base_efficiency = st.number_input("Base efficiency (sellable % of GFA)", min_value=0.01, max_value=1.0, value=0.85, step=0.01)

with st.sidebar.expander("Revenues & Base Costs", expanded=True):
...
with st.sidebar.expander("Overlays & Policy", expanded=True):
    ...
    inclusionary_cap_price_sqm = st.number_input("Affordable cap price (R/m²)", min_value=0.0, value=12000.0, step=250.0)
...
with st.sidebar.expander("Risk, Fees, DCs", expanded=True):
...
with st.sidebar.expander("Finance/Marketing & Profit", expanded=True):
    ...
    # ✅ Key patch: finance default 1.5% and properly scaled
...
overlays = Overlays(
    ...
    inclusionary_cap_price_sqm=inclusionary_cap_price_sqm,
    ...
    finance_marketing_rate=legacy_finance_marketing_rate,
...
if show_debug:
    ...
    st.sidebar.caption(f"DEBUG legacy_blended={overlays.finance_marketing_rate:.4f}")
...
try:
    ...
    result = model.calculate_rlv(
        exit_price_sqm=exit_price_sqm,
        build_cost_sqm=build_cost_sqm,
        include_affordable=inclusionary_enabled,
    )
...
c3.metric("Residual Land Value (before acquisition)", _money(result["residual_land_value"]))
c4.metric("Inclusionary", result["inclusionary_hit"])
...
is_viable_after = bool(audit["outputs"]["is_viable_after_acquisition_costs"])

l1, l2, l3, l4 = st.columns(4)
l1.metric("Binding constraint", binding)
l2.metric("Total bulk (GFA m²)", _num(total_bulk))
l3.metric("Sellable area (m²)", _num(sellable_area))
l4.metric("Viable (before / after acquisition)", f"{'Yes ✅' if is_viable else 'No ❌'} / {'Yes ✅' if is_viable_after else 'No ❌'}")

t1, t2, t3 = st.columns(3)
t1.metric("RLV per m² plot", _money(rlv / plot_size))
t2.metric("RLV per m² bulk", _money(rlv / total_bulk if total_bulk > 0 else 0.0))
t3.metric("RLV after acquisition", _money(rlv_after))
...
k5.metric("Incentives", _money(audit["costs"]["incentives_total"]))
